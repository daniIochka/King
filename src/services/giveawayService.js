...
export async function checkGiveaways(client) {
  try {
    ...
    logger.info(`Участники розыгрыша ${endedGiveaways.length}`);
    ...
    for (const giveawayRecord of endedGiveaways) {
      try {
        ...
        if (winners.length > 0) {
          const winnerAnnouncement = `🎉 Победил ${winnerMentions} Он выйграл **${giveaway.prize || 'giveaway'}**! Свяжитесь с организатором <@${giveaway.hostId}> для получения приза.`;
